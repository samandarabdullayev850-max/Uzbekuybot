...
async def cb_search(update,context):
    ...
    if d.startswith("s_city_"):
        s["city"]=d.replace("s_city_",""); context.user_data["search"]=s
        ...
        s["deal_type"]=d.replace("s_deal_",""); context.user_data["search"]=s
        ...
        s["rooms"]=d.replace("s_rooms_",""); context.user_data["search"]=s
        ...
        s["price_min"],s["price_max"]=PRICE_RANGES[idx]; context.user_data["search"]=s
...
async def show_results(q,context,lang,page):
    ...
    if not listings:
        await q.edit_message_text(t["no_results"]); return
    nav=[]
...
async def add_start(update,context):
    ...
    lang=u["language"] if u else "uz"
    t=tx(lang)
    context.user_data["nl"]={}; context.user_data["nl_step"]="deal"; context.user_data["nl_photos"]=[]
...
async def cb_listing(update,context):
    ...
    if d=="nl_cancel":
        context.user_data.pop("nl",None); context.user_data.pop("nl_step",None)
        await q.message.delete(); return
    if d.startswith("nl_deal_"):
        nl["deal_type"]=d.replace("nl_deal_",""); context.user_data["nl"]=nl; context.user_data["nl_step"]="city"
        ...
        nl["city"]=d.replace("nl_city_",""); context.user_data["nl"]=nl; context.user_data["nl_step"]="rooms"
        ...
        nl["rooms"]=int(d.replace("nl_rooms_","")); context.user_data["nl"]=nl; context.user_data["nl_step"]="price"
        await q.edit_message_text(t["ask_price"])
    elif d=="nl_skip_desc":
        nl["description"]=None; context.user_data["nl"]=nl; context.user_data["nl_step"]="photos"
        ...
        context.user_data["nl_step"]="address"; await q.edit_message_text(t["ask_address"])
    elif d=="nl_photos_done":
        context.user_data["nl_step"]="address"; await q.message.reply_text(t["ask_address"])
    elif d=="nl_skip_addr":
        nl["address"]=None; context.user_data["nl"]=nl; context.user_data["nl_step"]="phone"
...
async def msg_handler(update,context):
    ...
    if step=="price":
        ...
        if not text.isdigit():
            await update.message.reply_text("Narxni raqamda kiriting:"); return
        nl["price"]=int(text); context.user_data["nl"]=nl; context.user_data["nl_step"]="description"
        ...
        nl["description"]=update.message.text; context.user_data["nl"]=nl; context.user_data["nl_step"]="photos"
        ...
        if update.message.photo:
            ...
            photos.append(update.message.photo[-1].file_id); context.user_data["nl_photos"]=photos
            if len(photos)>=10:
                context.user_data["nl_step"]="address"; await update.message.reply_text(t["ask_address"])
            else:
        ...
        nl["address"]=update.message.text; context.user_data["nl"]=nl; context.user_data["nl_step"]="phone"
        await update.message.reply_text(t["ask_phone"])
    elif step=="phone":
        phone=update.message.contact.phone_number if update.message.contact else update.message.text
        nl["phone"]=phone; nl["photos"]=context.user_data.get("nl_photos",[]); nl["source"]="bot"; nl["user_id"]=u["id"]; nl["is_active"]=False
        add_listing(nl)
        context.user_data.pop("nl",None); context.user_data.pop("nl_step",None); context.user_data.pop("nl_photos",None)
...
async def settings_start(update,context):
    ...
    t=tx(lang)
    sub=get_sub(u["id"]) or {}
    is_active=sub.get("is_active",True); only_cheap=sub.get("only_cheap",False)
    freq=sub.get("notify_freq","daily"); daily_limit=sub.get("daily_limit",5)
    fl=NOTIFY_FREQ[lang]; limit_label=t["limit_unlimited"] if daily_limit==0 else f"{daily_limit} ta"
...
async def cb_settings(update,context):
    ...
    if d=="set_notif": save_sub(u["id"],{"is_active":not sub.get("is_active",True)})
        ...
        fl=NOTIFY_FREQ[lang]
        await q.edit_message_text(t["choose_freq"],reply_markup=InlineKeyboardMarkup([[InlineKeyboardButton(fl["instant"],callback_data="sf_instant")],[InlineKeyboardButton(fl["daily"],callback_data="sf_daily")],[InlineKeyboardButton(fl["weekly"],callback_data="sf_weekly")]])); return
    elif d.startswith("sf_"): save_sub(u["id"],{"notify_freq":d.replace("sf_","")})
    elif d=="set_limit":
        await q.edit_message_text(t["choose_limit"],reply_markup=InlineKeyboardMarkup([[InlineKeyboardButton(t["limit_unlimited"] if l==0 else f"{l} ta",callback_data=f"sl_{l}")] for l in DAILY_LIMITS])); return
    elif d.startswith("sl_"): save_sub(u["id"],{"daily_limit":int(d.replace("sl_",""))})
    elif d=="set_lang":
        await q.edit_message_text(t["choose_language"],reply_markup=InlineKeyboardMarkup([[InlineKeyboardButton("O'zbek",callback_data="lang_uz"),InlineKeyboardButton("Русский",callback_data="lang_ru"),InlineKeyboardButton("English",callback_data="lang_en")]])); return
    sub2=get_sub(u["id"]) or {}
    is_active=sub2.get("is_active",True); only_cheap=sub2.get("only_cheap",False)
    freq=sub2.get("notify_freq","daily"); daily_limit=sub2.get("daily_limit",5)
    fl=NOTIFY_FREQ[lang]; limit_label=t["limit_unlimited"] if daily_limit==0 else f"{daily_limit} ta"
    kb=InlineKeyboardMarkup([[InlineKeyboardButton(t["notif_on"] if is_active else t["notif_off"],callback_data="set_notif")],[InlineKeyboardButton(t["only_cheap_on"] if only_cheap else t["only_cheap_off"],callback_data="set_cheap")],[InlineKeyboardButton(fl[freq],callback_data="set_freq")],[InlineKeyboardButton(f"Limit: {limit_label}",callback_data="set_limit")],[InlineKeyboardButton("Tilni ozgartirish",callback_data="set_lang")]])
